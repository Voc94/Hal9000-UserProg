...
)
{
    ...
    while (*start != '\0' && index < Process->NumberOfArguments) {
        ...
        if (*start == ' ') {
            *start = '\0'; // Null terminate argument
            start++;
        }
    }
    ...
    status = MmuGetSystemVirtualAddressForUserBuffer(
        (char*)PtrDiff(InitialStack, STACK_DEFAULT_SIZE),
        STACK_DEFAULT_SIZE,
    ...
    PVOID StackTop = PtrOffset(BaseStackKernelAddress, STACK_DEFAULT_SIZE);
    ...
    for (int i = Process->NumberOfArguments - 1; i >= 0; --i) {
        WORD argLen = (WORD)(strlen(argv[i]) + 1);

        // Move stack iterator backward and copy argument
        StackIterator = (PVOID)((char*)StackIterator - argLen);
        memcpy(StackIterator, argv[i], argLen);
        stackArgv[i] = (QWORD)InitialStack - ((QWORD)StackTop - (QWORD)StackIterator);
    }
    ...
    for (DWORD i = 0; i < index; i++) {
        ...
        *((QWORD*)StackIterator) = stackArgv[i];
    }

    QWORD Argv0 = PtrDiff(InitialStack, ((QWORD)StackTop - (QWORD)StackIterator));
    ...
    *(QWORD*)StackIterator = Argv0; // Store the address of the argv array

    //// argc (number of arguments)
    StackIterator = (PVOID)((char*)StackIterator - sizeof(QWORD));
    *(QWORD*)StackIterator = (QWORD)Process->NumberOfArguments;
    ...
    *ResultingStack = (PVOID)((char*)InitialStack - ((PBYTE)StackTop - (PBYTE)StackIterator));
